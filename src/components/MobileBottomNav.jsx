              ...
              <Link key={item.to} to={item.to} className="block self-end">
                <div
                  className={`relative flex min-h-[64px] flex-col items-center justify-center gap-1 transition-all duration-300 ${
                    item.featured
                      ? 'mobile-nav-featured mx-1 min-h-[72px] translate-y-[-8px] rounded-[24px] border border-primary/25 bg-[linear-gradient(180deg,rgba(34,48,93,0.98),rgba(12,19,40,0.96))] text-primary shadow-[0_14px_34px_rgba(0,0,0,0.4)]'
                      : active
                ...
                >
                  {active ? (
                    <div className={`absolute inset-0 border border-primary/25 bg-primary/10 shadow-[0_0_18px_rgba(59,130,246,0.18)] ${item.featured ? 'rounded-[24px]' : 'rounded-2xl'}`} />
                  ) : null}
